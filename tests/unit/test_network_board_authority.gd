...
func test_advance_turn_only_from_active_player_peer() -> void:
	...
	assert_true(board.try_advance_turn(slots, active_slot.owning_peer_id, active_id))
	...
	assert_false(
		board.try_advance_turn(slots, wrong_slot.owning_peer_id, next_active)
	)
	assert_true(board.try_advance_turn(slots, next_slot.owning_peer_id, next_active))
...
func test_rejects_advance_for_wrong_player_id() -> void:
	...
	assert_false(board.try_advance_turn(slots, slots[1].owning_peer_id, inactive_id))
	assert_eq(board.board_stub.turn_index, 0)
...
func test_export_and_load_round_trip_preserves_hash() -> void:
	...
	var slots := _make_slots()
	board.reset_for_slots(slots)
	board.try_advance_turn(slots, slots[0].owning_peer_id, slots[0].player_id)
