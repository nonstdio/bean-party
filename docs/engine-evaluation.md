...
**No engine has been selected.** The project should choose after a small vertical-slice comparison, not from brand familiarity alone. The first priority is a sustainable contributor workflow for a party-game prototype, not maximum graphical power.
...
- deterministic-enough local rules and a path to online play later, without making online networking a first-slice dependency;
...
Run a short, time-boxed vertical slice before selecting an engine. Build the same tiny experience in the leading candidates:
...
Choose the engine only after at least two potential contributors have tried the slice. Record the engine version, language choice, project layout, build commands, and minigame API in a decision record at that time.

## Questions the group must answer first

- What is the first release platform: Windows desktop, web, or something else?
- Is remote online multiplayer a launch requirement or a later milestone?
- Are the first boards and minigames primarily 2D, 3D, or deliberately mixed?
- What languages and engines can likely contributors already use?
